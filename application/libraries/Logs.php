...
class Logs {
        ...
        public function get_attempt_logs($log_directory, $job_id, $user, $node_id, $cont_id){
            ...
            $logs["syslog"] = substr($attempt_logs, ($syslog_pos + 6), ($next_cont_pos - $syslog_pos - 15));
            
            
            return $logs;
        }
    
        public function get_spark_logs($log_directory, $job_id, $user, $node_id, $cont_id){
            $logs=array();
            ...
            $next_cont_pos = strpos($attempt_logs, '&container');
            $stderr_pos = strpos($attempt_logs, "stderr");
            $stdout_pos = strrpos($attempt_logs, "stdout");

            $logs["stderr"] = substr($attempt_logs, ($stderr_pos + 6), ($stdout_pos - $stderr_pos - 6));
            $logs["stdout"] = substr($attempt_logs, ($stdout_pos + 6), ($next_cont_pos - $stdout_pos - 15));
